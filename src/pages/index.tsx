  ...
  useEffect(() => {
    ...
    const requestOptions = {
      ...
      body: JSON.stringify({
        ...
        userType: session?.user?.name ?? UserType.GUEST,
      }),
    };
      ...
      .then((responseData) => {
        ...
        switch (user) {
          ...
          case UserType.ORGANIZATION:
            setTab0(
              responseData.filter((event) => new Date(event.time) > currentDate)
            );
            setVolunteerEvents(
              responseData.filter((event) => new Date(event.time) > currentDate)
            );
      ...
      <Group justify="space-evenly" style={{ margin: 25 }}>
        {volunteerEvents.length > 0 ? (
          volunteerEvents?.map((event, index) => {
            return (
              <VolunteerEventCard attending={false} key={index} event={event} />
            );
          })
        ) : (
